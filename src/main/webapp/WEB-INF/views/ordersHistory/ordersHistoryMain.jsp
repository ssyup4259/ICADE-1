	...
	<div class="container-fluid" style="background-color: #F2F1F0; padding-top: 50px; padding-bottom: 50px;">
		<div class="container">
			...
			<form method="get" id="OrderHistoryFormId" name="OrderHistoryForm" action="orderHistory.action?pageNum=${pageNum}&startDay=${startDate}">
				<div class="container-fluid" style="border: 3px solid #8F9493; border-radius: 6px; padding-bottom: 30px;">
					<h2>검색기간설정</h2>
					...
					<div class="row">
						...
						<!-- 조회 버튼 -->
						<div class="col-sm-2">
							<input type="submit" class="btnGreen" value="조회" id="order_search_btn" style="width: 100px; height: 30px;">
						</div>
					</div>
					<div>
						<br>
			...
			<div class="container-fluid" style="border: 3px solid #8F9493; border-radius: 6px; padding-bottom: 30px;">
				...
				<table style="width: 100%;">
					...
					<thead>
						<tr>
							<th scope="col">주문일자[주문번호]<br>[상세정보]
							</th>
							<th scope="col">이미지</th>
							...
							<c:forEach var="dto" items="${map.value}" varStatus="i">
								<tr>
									<td class="gubun">
										...
										</p> <a href="#none" class="line" onclick="location.href='ordersHistoryDetail.action?o_num=${dto.getO_NUM()}'">[상세정보]</a>
									</td>
									<td><a href="/icade/goods/goodsArticle.action?G_NUM=${dto.getGD_NUM()}"> <img src="/icade/resources/goodsImage/${dto.getG_SAVEFILENAME()}" width="80" height="80"></a></td>
									<td><a href="/icade/goods/goodsArticle.action?G_NUM=${dto.getGD_NUM()}"> <strong>${dto.getOD_NAME()}</strong></a>
